...
# Make the repo root importable so `from synapse_model import ...` works
# regardless of cwd or how the script was launched.
...
from synapse_model import (
    SynapseGPT,
    BLOCK_SIZE, STRIDE, EMBED_DIM, NUM_LAYERS, NUM_HEADS, NUM_KV_HEADS,
...
# Imported from synapse_model above: BLOCK_SIZE, STRIDE, EMBED_DIM, NUM_LAYERS,
# NUM_HEADS, NUM_KV_HEADS, FF_HIDDEN_DIM, ROPE_BASE, RMSNORM_EPS, GRAD_CHECKPOINT.
...
# ==================== 4. MODEL DEFINITION ====================
# Model classes (SynapseGPT, TransformerBlock, CausalGQA, SwiGLU, RMSNorm,
# precompute_rope, apply_rope) are imported from `synapse_model` at the top
# of this file. They were extracted so SFT and eval can share the same
# definition without code duplication.
...
model = SynapseGPT(VOCAB_SIZE, gradient_checkpointing=GRAD_CHECKPOINT).to(device)
