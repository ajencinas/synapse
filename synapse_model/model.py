"""SynapseGPT model — 2B param GQA transformer with RoPE + RMSNorm + SwiGLU.

Extracted from pretrain/train.py so the same model definition can be reused
by pretrain, SFT, and eval without code duplication.

The only behavioral change vs the pre-refactor inline definition is that
`gradient_checkpointing` is now a constructor kwarg (default False) instead
of a module-level global read at construction time. All other defaults match
the original architecture constants bit-for-bit.
"""
import torch
...
ROPE_BASE       = 10000.0
RMSNORM_EPS     = 1e-5
GRAD_CHECKPOINT = False         # default off; pretrain on 95 GB has headroom, SFT enables it
...
class RMSNorm(nn.Module):
    ...
    # Graph-break: Inductor was fusing RMSNorm fwd+bwd into one Triton kernel
    # that wanted >160 KB of shared memory and couldn't compile on Blackwell.
...
class SynapseGPT(nn.Module):
    def __init__(self, vocab_size, *, gradient_checkpointing=False):
        super().__init__()
